...
export class PrinterExpression extends BasePrinter {
  printAssignOp = ({ node }) => node.name;
  printBinaryOperation = ({ node, path, print }) => {
    const parts = [];
    if (node.left !== null)
      parts.push(path.call(print, "left"));
    if (node.operator !== null)
      parts.push(this.space, node.operator);
    if (node.right !== null)
      parts.push(this.builders.line, path.call(print, "right"));
    return this.builders.group(parts);
  };
  printAssignment = ({ node, path, print }) => {
    const parts = [];
    if (node.left !== null)
      parts.push(path.call(print, "left"));
    if (node.operator !== null)
  ...
  printBooleanLiteral = ({ node }) => node.value === true ? "true" : "false";
  printConditional = ({ path, print }) => {
  ...
  printUnicodeStringLiteral = ({ node }) => [
    "unicode",
    this.literal(node.value)
  ];
  printUserDefinableOperator = ({ node }) => node.name;
}
