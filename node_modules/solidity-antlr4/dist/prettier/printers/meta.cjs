...
exports.PrinterMeta = void 0;
var _base = require("./base.cjs");
class PrinterMeta extends _base.BasePrinter {
  printDataLocation = ({
    node
  }) => node.name;
  printIdentifierPath = ({
  ...
  printImportDirective = ({
    ...
    if (node.unitAlias) {
      ...
      if (node.importAll) {
        result = this.builders.join(this.space, ["import", "*", "as", unitAlias, "from", importPath]);
      } else {
        result = this.builders.join(this.space, ["import", importPath, "as", unitAlias]);
      }
    } else if (node.symbolAliases.length) {
      result = this.builders.join(this.space, ["import", this.block(this.paramater(path.map(print, "symbolAliases"))), "from", importPath]);
    } else {
      result = this.builders.join(this.space, ["import", importPath]);
    }
    return [result, this.semi];
  };
  printInheritanceSpecifier = ({
    node,
    path,
    print
  }) => {
  ...
  printPragmaDirective = ({
    ...
    return this.builders.group(["pragma", line, literal, this.semi], {
      id: groupId
    });
  };
  printSourceUnit = ({
  ...
  printUsingAliases = ({
    ...
    if (node.operator !== null) parts.push(this.space, "as", this.space, path.call(print, "operator"));
    return parts;
  };
  printUsingDirective = ({
    node,
...
exports.PrinterMeta = PrinterMeta;