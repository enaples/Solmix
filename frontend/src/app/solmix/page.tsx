...
export default function SolmixHome() {
    ...
    // Fetch data should reflect the http test following:
    // @baseUrl = http://localhost:8000

    // POST {{baseUrl}}/vulnerability-analysis
    ...
    useEffect(() => {
        if (code === "no code yet") return;

        const fetchData = async () => {
            setShowSpinner(true);
            const response = await fetch("http://localhost:8000/vulnerability-analysis", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
                // The code should be contained in the keyword "solidityCode"
                body: JSON.stringify({
                    "solidityCode": code,
                }),
            });
