...
export function removeFromDropdown(
    ...
    switch (type) {
        ...
        case "assign_values_to_variable_array":
            solidityArrays = solidityArrays.filter(
                (item) => item.name !== name
            );
            updateArraysDropdown();
            console.log("🗑️ Vecchio array rimosso:", name);
        ...
        case "contract_structures":
            solidityStructs = solidityStructs.filter(
                (item) => item.name !== name
            );
            updateStructsDropdown();
        ...
        case "mapping":
            solidityMappings = solidityMappings.filter(
                (item) => item.name !== name
            );
            updateMappingsDropdown();
            console.log("🗑️ Vecchio mapping rimosso:", name);
        ...
        case "event":
            solidityEvents = solidityEvents.filter(
                (item) => item.name !== name
            );
            updateEventsDropdown();
            console.log("🗑️ Vecchio Event rimosso:", name);
...
// addToDropdown
export function addToDropdown(name: string, type: SolidityDropdownType): void {
    switch (type) {
        case "array":
        case "assign_values_to_variable_array":
...
// ## UpdateDropdown
// String
export function getSolidityStringVariable(
    name: string
): SolidityVariable | undefined {
    return solidityStringVariables.find((variable) => variable.name === name);
}

export function getSolidityStringConstantsVariable(
    name: string
): SolidityVariable | undefined {
    return solidityStringConstantsVariables.find(
        (variable) => variable.name === name
    );
}

export function getSolidityStringImmutablesVariable(
    name: string
): SolidityVariable | undefined {
    return solidityStringImmutablesVariables.find(
        (variable) => variable.name === name
    );
}

// Uint
export function getSolidityUintVariable(
    name: string
): SolidityVariable | undefined {
    return solidityUintVariables.find((variable) => variable.name === name);
}

export function getSolidityUintConstantsVariable(
    name: string
): SolidityVariable | undefined {
    return solidityUintConstantsVariables.find(
        (variable) => variable.name === name
    );
}

export function getSolidityUintImmutablesVariable(
    name: string
): SolidityVariable | undefined {
    return solidityUintImmutablesVariables.find(
        (variable) => variable.name === name
    );
}

export function getSolidityUint256Variable(
    name: string
): SolidityVariable | undefined {
    return solidityUint256Variables.find((variable) => variable.name === name);
}

export function getSolidityUint256ConstantsVariable(
    name: string
): SolidityVariable | undefined {
    return solidityUint256ConstantsVariables.find(
        (variable) => variable.name === name
    );
}

export function getSolidityUint256ImmutablesVariable(
    name: string
): SolidityVariable | undefined {
    return solidityUint256ImmutablesVariables.find(
        (variable) => variable.name === name
    );
}

export function getSolidityUint8Variable(
    name: string
): SolidityVariable | undefined {
    return solidityUint8Variables.find((variable) => variable.name === name);
}

export function getSolidityUint8ConstantsVariable(
    name: string
): SolidityVariable | undefined {
    return solidityUint8ConstantsVariables.find(
        (variable) => variable.name === name
    );
}

export function getSolidityUint8ImmutablesVariable(
    name: string
): SolidityVariable | undefined {
    return solidityUint8ImmutablesVariables.find(
        (variable) => variable.name === name
    );
}

// Int
export function getSolidityIntVariable(
    name: string
): SolidityVariable | undefined {
    return solidityIntVariables.find((variable) => variable.name === name);
}

export function getSolidityIntConstantsVariable(
    name: string
): SolidityVariable | undefined {
    return solidityIntConstantsVariables.find(
        (variable) => variable.name === name
    );
}

export function getSolidityIntImmutablesVariable(
    name: string
): SolidityVariable | undefined {
    return solidityIntImmutablesVariables.find(
        (variable) => variable.name === name
    );
}

// Address
export function getSolidityAddressVariable(
    name: string
): SolidityVariable | undefined {
    return solidityAddressVariables.find((variable) => variable.name === name);
}

export function getSolidityAddressConstantsVariable(
    name: string
): SolidityVariable | undefined {
    return solidityAddressConstantsVariables.find(
        (variable) => variable.name === name
    );
}

export function getSolidityAddressImmutablesVariable(
    name: string
): SolidityVariable | undefined {
    return solidityAddressImmutablesVariables.find(
        (variable) => variable.name === name
    );
}

// Boolean
export function getSolidityBoolVariable(
    name: string
): SolidityVariable | undefined {
    return solidityBoolVariables.find((variable) => variable.name === name);
}

export function getSolidityBoolConstantsVariable(
    name: string
): SolidityVariable | undefined {
    return solidityBoolConstantsVariables.find(
        (variable) => variable.name === name
    );
}

export function getSolidityBoolImmutablesVariable(
    name: string
): SolidityVariable | undefined {
...
// Bytes - Bytes32
export function getSolidityBytesVariable(
    name: string
): SolidityVariable | undefined {
    return solidityBytesVariables.find(
        (variable: SolidityVariable) => variable.name === name
    );
}

export function getSolidityBytesConstantsVariable(
    name: string
): SolidityVariable | undefined {
    return solidityBytesConstantsVariables.find(
        (variable: SolidityVariable) => variable.name === name
    );
}

export function getSolidityBytesImmutablesVariable(
    name: string
): SolidityVariable | undefined {
...
export function getSolidityBytes32Variable(
    name: string
): SolidityVariable | undefined {
    return solidityBytes32Variables.find((variable) => variable.name === name);
}

export function getSolidityBytes32ConstantsVariable(
    name: string
): SolidityVariable | undefined {
    return solidityBytes32ConstantsVariables.find(
        (variable) => variable.name === name
    );
}

export function getSolidityBytes32ImmutablesVariable(
    name: string
): SolidityVariable | undefined {
    return solidityBytes32ImmutablesVariables.find(
        (variable) => variable.name === name
    );
}
...
export function updateEventsDropdown(): void {
    ...
    workspace.getAllBlocks(false).forEach((block: Blockly.Block) => {
        if (block.type === "emit_event") {
            //variables_get_events
            const menu: [string, string][] = solidityEvents.length
            ...
            if (dropdown && "menuGenerator_" in dropdown) {
                ...
                dropdown.setValue(
                    eventNames.includes(block.getFieldValue("VAR"))
                        ? block.getFieldValue("VAR")
                        : ""
                ); //(eventNames.includes(current) ? current : "");
    ...
    workspace.getAllBlocks(false).forEach((block) => {
        if (block.type === "getter_mappings") {
            ...
            if (dropdown && "menuGenerator_" in dropdown) {
                ...
                dropdown.setValue(
                    mappingNames.includes(block.getFieldValue("VAR"))
                        ? block.getFieldValue("VAR")
                        : ""
                ); //(eventNames.includes(current) ? current : "");
                //dropdown.setValue(eventNames.includes(dropdown) ? dropdown : "");
...
export function getSolidityMapping(name: string): { name: string } | undefined {
    ...
    workspace.getAllBlocks(false).forEach((block) => {
        if (block.type === "array_values") {
            ...
            if (dropdown && "menuGenerator_" in dropdown) {
                ...
                dropdown.setValue(
                    arrayNames.includes(block.getFieldValue("VAR"))
                        ? block.getFieldValue("VAR")
                        : ""
                ); //(eventNames.includes(current) ? current : "");
                //dropdown.setValue(eventNames.includes(dropdown) ? dropdown : "");
...
export function getSolidityArray(name: string): { name: string } | undefined {
    ...
    workspace.getAllBlocks(false).forEach((block) => {
        if (
            ...
            if (dropdown && "menuGenerator_" in dropdown) {
                ...
                dropdown.setValue(
                    structNames.includes(block.getFieldValue("VAR"))
                        ? block.getFieldValue("VAR")
                        : ""
                ); //(eventNames.includes(current) ? current : "");
                //dropdown.setValue(eventNames.includes(dropdown) ? dropdown : "");
...
export function getSolidityStruct(name: string): { name: string } | undefined {
    ...
    workspace.getAllBlocks(false).forEach((block) => {
        if (block.type === "struct_push") {
            ...
            if (dropdown && "menuGenerator_" in dropdown) {
                ...
                dropdown.setValue(
                    structArrayNames.includes(block.getFieldValue("VAR"))
                        ? block.getFieldValue("VAR")
                        : ""
                ); //(eventNames.includes(current) ? current : "");
                //dropdown.setValue(eventNames.includes(dropdown) ? dropdown : "");
    ...
    workspace.getAllBlocks(false).forEach((block) => {
        if (block.type === "variables_get_modifiers") {
            ...
            if (dropdown && "menuGenerator_" in dropdown) {
                ...
                dropdown.setValue(
                    modifierNames.includes(block.getFieldValue("VAR"))
                        ? block.getFieldValue("VAR")
                        : ""
                ); //(eventNames.includes(current) ? current : "");
                //dropdown.setValue(eventNames.includes(dropdown) ? dropdown : "");
    ...
    for (const block of blocks) {
        if (block.type === "new_struct_value") {
            ...
            if (selectedStruct === structName) {
                const attrDropdown = block.getField(
                    "ATTRIBUTE"
