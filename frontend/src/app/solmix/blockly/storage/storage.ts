...
const useBlocklyStorageImpl = ({
    ...
    const backupBlocks = useCallback(
        ...
        (
            ...
            xhr.onreadystatechange = () => {
                if (xhr.readyState === 4) {
                    if (xhr.status !== 200) {
                        onAlert(
                            `${DEFAULT_MESSAGES.HTTPREQUEST_ERROR}\nhttpRequest_.status: ${xhr.status}`
                        );
                    } else {
                        ...
                        if (name === "xml") {
                            ...
                            if (typeof window !== "undefined") {
                                ...
                                onAlert(
                                    DEFAULT_MESSAGES.LINK_ALERT.replace(
                                        "%1",
                            ...
                            if (!data.length) {
                                onAlert(
                                    DEFAULT_MESSAGES.HASH_ERROR.replace(
                                        "%1",
    ...
    const loadXml = useCallback(
        (xml: string, targetWorkspace: BlocklyWorkspace) => {
    ...
    const retrieveXml = useCallback(
        (key: string, targetWorkspace?: BlocklyWorkspace) => {
            const ws = targetWorkspace || getWorkspace();
            if (!ws) return;
