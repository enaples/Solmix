    ...
    this.appendDummyInput()
      ...
      .appendField(new Blockly.FieldDropdown((): [string, string][] => {
        return solidityStringVariables.length
          ? solidityStringVariables.map(v => [v.name, v.name] as [string, string])
          : [["", ""]];
      }), "VAR");
    ...
    this.appendDummyInput()
      ...
      .appendField(new Blockly.FieldDropdown(() => {
        return solidityStringConstantsVariables.length
          ? solidityStringConstantsVariables.map(v => [v.name, v.name])
          : [["", ""]];
      }), "VAR");

    this.setOutput(true, "String_Output_Const");
    this.setColour(330);
    this.setTooltip("Returns the value of a constant string variable you've defined.");
    ...
    this.appendDummyInput()
      ...
      .appendField(new Blockly.FieldDropdown(() => {
        return solidityStringImmutablesVariables.length
          ? solidityStringImmutablesVariables.map(v => [v.name, v.name])
          : [["", ""]];
      }), "VAR");

    this.setOutput(true, "String_Output_Imm");
    this.setColour(200);
    this.setTooltip("It returns the name of an immutable string variable you've defined.");
    this.setHelpUrl("");
  }
};
...
Blockly.Blocks['variables_get_s'] = {
  init: function (this: Blockly.Block) {
    this.appendDummyInput()
      ...
      .appendField(new Blockly.FieldDropdown(() => {
        return solidityStringVariables.length
          ? solidityStringVariables.map((v: any) => [v.name, v.name])
          : [["", ""]];
      }), "VAR");
