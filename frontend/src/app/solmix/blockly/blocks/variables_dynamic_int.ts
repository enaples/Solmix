    ...
    this.appendDummyInput()
      ...
      .appendField(new Blockly.FieldDropdown((): [string, string][] => {
        return solidityIntVariables.length
          ? solidityIntVariables.map(v => [v.name, v.name] as [string, string])
          : [["", ""]];
      }), "VAR");
    ...
    this.appendDummyInput()
      ...
      .appendField(new Blockly.FieldDropdown((): [string, string][] => {
        return solidityIntConstantsVariables.length
          ? solidityIntConstantsVariables.map(v => [v.name, v.name] as [string, string])
          : [["", ""]];
      }), "VAR");
    this.setOutput(true, "Value_Output_Const");
    this.setColour(330);
    this.setTooltip("It returns the name of a constant int variable you've defined.");
  }
};
    ...
    this.appendDummyInput()
      ...
      .appendField(new Blockly.FieldDropdown((): [string, string][] => {
        return solidityIntImmutablesVariables.length
          ? solidityIntImmutablesVariables.map(v => [v.name, v.name] as [string, string])
          : [["", ""]];
      }), "VAR");
    
    this.setOutput(true, "Value_Output_Imm");
    this.setColour(200);
    this.setTooltip("It returns the name of an immutable int variable you've defined.");
  }
};
