...
Blockly.Blocks["variables_get_address_constants"] = {
    init: function () {
            ...
            .appendField(
                new Blockly.FieldDropdown(function () {
                    return solidityAddressConstantsVariables.length
                        ? solidityAddressConstantsVariables.map((v) => [
                              v.name,
                              v.name,
                          ])
                        : [["", ""]];
        ...
        this.setColour(330);
        this.setTooltip(
            "It returns the name of a constant address variable you've defined."
        );
        this.setHelpUrl("");
    },
};
...
Blockly.Blocks["variables_get_address_immutables"] = {
    init: function () {
            ...
            .appendField(
                new Blockly.FieldDropdown(function () {
                    return solidityAddressImmutablesVariables.length
                        ? solidityAddressImmutablesVariables.map((v) => [
                              v.name,
                              v.name,
                          ])
                        : [["", ""]];
        ...
        this.setTooltip(
            "It returns the name of an immutable address variable you've defined."
        );
    },
};
