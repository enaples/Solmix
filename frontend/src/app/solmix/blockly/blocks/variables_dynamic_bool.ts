        ...
        this.appendDummyInput()
            ...
            .appendField(
                new Blockly.FieldDropdown((): [string, string][] => {
                    return solidityBoolVariables.length
                        ? solidityBoolVariables.map((v: { name: string }) => [
                              v.name,
                              v.name,
                          ])
                        : [["", ""]];
...
Blockly.Blocks["variables_get_bool_constants"] = {
    init: function (): void {
            ...
            .appendField(
                new Blockly.FieldDropdown((): [string, string][] => {
                    return solidityBoolConstantsVariables.length
                        ? solidityBoolConstantsVariables.map(
        ...
        this.setColour(330);
        this.setTooltip(
            "It returns the name of a constant boolean variable you've defined."
        );
    },
};
...
Blockly.Blocks["variables_get_bool_immutables"] = {
    init: function (): void {
            ...
            .appendField(
                new Blockly.FieldDropdown(function (): [string, string][] {
                    return solidityBoolImmutablesVariables.length
                        ? solidityBoolImmutablesVariables.map(
                              (v: { name: string }) => [v.name, v.name]
                          )
                        : [["", ""]];
                }),
        ...
        this.setColour(200);
        this.setTooltip(
            "It returns the name of an immutable boolean variable you've defined."
        );
    },
};
