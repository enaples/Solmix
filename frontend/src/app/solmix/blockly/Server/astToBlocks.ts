...
// Funzione principale
export function createBlocksFromAST(
    ast: any,
    ...
    function traverse(node: any, parentBlockId: string | null = null): void {
        ...
        switch (node.type) {
            case "SourceUnit": {
                ...
                if (node.nodes) {
                    const sortedChildren = node.nodes
                        .slice()
                        .sort((a: any, b: any) => {
                            if (a.range && b.range)
            ...
            case "ContractDefinition": {
                ...
                const isFieldValue = baseContracts
                    .map((b: any) => b.baseName.name)
                    .join(", ");
            ...
            case "FunctionDefinition": {
                ...
                if (node.functionKind === "constructor") {
                    ...
                    console.log(
                        `Constructor - ID: ${constructorBlock.id}, Parent: ${parentBlockId}`
                    );
                    ...
                    node.parameters?.forEach((param: any) => {
                        ...
                        const completeName = dataLocation
                            ? `${dataLocation} ${name}`
                            : name;

                        const blockType = returnType[typeName]
                            ? "func_inputs"
                            : "func_inputs_black";
                    ...
                    if (node.body?.statements) {
                        const sortedStatements = node.body.statements
                            .slice()
                            .sort((a: any, b: any) => b.range[0] - a.range[0]);
                            ...
                            if (
                                ...
                                if (variableTypes[varName]) {
                                    ...
                                    if (setterType) {
                                        ...
                                        blocklyJson.blocks.blocks.push(
                                            setblock
                                        );
                                    ...
                                    console.warn(
                                        `⚠️ Variabile ${varName} non trovata in variableTypes`
                                    );
                    ...
                    const functionBlock: BlocklyBlock = {
                        ...
                        fields: {
                            NAME: node.name.name,
                            ACCESS:
                                visibilityMap[node.visibility] || "TYPE_PUBLIC",
                            VIEW:
                    ...
                    functionBlock.data = JSON.stringify({
                        parentId: functionBlock.parent,
                    });
                    ...
                    node.modifiers?.forEach((modifier: any) => {
                        ...
                        const ModifierGetterBlockId: string =
                            generateUniqueId();
                        ...
                        ModifierGetterBlock.data = JSON.stringify({
                            parentId: ModifierGetterBlock.parent,
                        });
                        blocklyJson.blocks.blocks.push(ModifierGetterBlock);

                        modifier.arguments?.forEach((param: any) => {
                            const functionParameterBlockId: string =
                                generateUniqueId();
                            const paramBlock: BlocklyBlock = {
                                type: "func_inputs",
                                fields: {
                            ...
                            paramBlock.data = JSON.stringify({
                                parentId: paramBlock.parent,
                            });
                            blocklyJson.blocks.blocks.push(paramBlock);
                        });
                    });
                    ...
                    node.parameters?.forEach((param: any) => {
                        ...
                        if (isKnownType) {
                            ...
                            paramBlockBlack.data = JSON.stringify({
                                parentId: paramBlockBlack.parent,
                            });
                    ...
                    node.returnParameters?.forEach((value: any) => {
                        const functionReturnValueBlockId: string =
                            generateUniqueId();
                        const returnValueBlock: BlocklyBlock = {
                            type: "func_returnValues",
                            fields: {
                                TYPE:
                                    returnType[value.typeName.name] ||
                                    "TYPE_UNKNOWN",
                        ...
                        returnValueBlock.data = JSON.stringify({
                            parentId: returnValueBlock.parent,
                        });
                    ...
                    if (node.body && node.body.statements) {
                        ...
                        sortedStatements.forEach((statement: any) => {
                            if (
                                ...
                                if (
                                    statement.expression.expression.name ===
                                    "require"
                                ) {
                                    console.log(
                                        "require inside method rilevata!"
                                    );
                                    ...
                                    if (requireNode.arguments?.length > 0) {
                                        const requireConditionBlockMethodId =
                                            generateUniqueId();
                                        const requireConditionMethodBlock: BlocklyBlock =
                                            {
                                                ...
                                                fields: {
                                                    MESSAGE:
                                                        requireNode.arguments[1]
                                                            ?.value || "",
                                                },
                                                id: requireConditionBlockMethodId,
                                                parent: functionBlock.id,
                                            };
                                        requireConditionMethodBlock.data =
                                            JSON.stringify({
                                                parentId:
                                                    requireConditionMethodBlock.parent,
                                            });
                                        blocklyJson.blocks.blocks.push(
                                            requireConditionMethodBlock
                                        );

                                        const requireArg =
                                            requireNode.arguments[0];
                                        ...
                                        if (!conditionExpression) {
                                            console.error(
                                                "❌ Errore: impossibile trovare l'espressione logica nel require()"
                                            );
                                        } else {
                                            console.log(
                                                "✅ conditionExpression trovata:",
                                                conditionExpression
                                            );
                                        }
                                        ...
                                        if (
                                            ...
                                            const solidityOperator =
                                                conditionExpression.operator;
                                            console.log(
                                                "Operatore trovato nel require:",
                                                solidityOperator
                                            );
                                            ...
                                            if (solidityOperator === "!") {
                                                ...
                                                requireConditionBlock.data =
                                                    JSON.stringify({
                                                        parentId:
                                                            requireConditionBlock.parent,
                                                    });
                                                blocklyJson.blocks.blocks.push(
                                                    requireConditionBlock
                                                );
                                                ...
                                                if (
                                                    right.type === "IndexAccess"
                                                ) {
                                                    console.log(
                                                        "IndexAccess Riconosciuto"
                                                    );
                                                    const baseExpr =
                                                    ...
                                                    inputBlockRight.data =
                                                        JSON.stringify({
                                                            parentId:
                                                                inputBlockRight.parent,
                                                        });
                                                    blocklyJson.blocks.blocks.push(
                                                        inputBlockRight
                                                    );
                                                    console.log(
                                                        "inptBlockRight for UnaryOperation created!"
                                                    );
                                                }
                                            } else if (
                                                solidityOperator === "==" ||
                                                ...
                                                const inputBlockLeftId =
                                                    generateUniqueId();
                                                let inputBlockLeft: BlocklyBlock;

                                                if (
                                                    "name" in left &&
                                                    findVariable(ast, left.name)
                                                ) {
                                                    const type =
                                                        findVariableType(
                                                            ast,
                                                    ...
                                                    if (blockType) {
                                                        ...
                                                        throw new Error(
                                                            `❌ Tipo non supportato o non trovato per la variabile ${left.name}`
                                                        );
                                                    }
                                                } else {
                                                ...
                                                inputBlockLeft.data =
                                                    JSON.stringify({
                                                        parentId:
                                                            inputBlockLeft.parent,
                                                    });
                                                blocklyJson.blocks.blocks.push(
                                                    inputBlockLeft
                                                );
                                                console.log(
                                                    "✅ inputBlockLeft for BinaryOperation created!"
                                                );
                                            } else if (
                                                "name" in conditionExpression
                                            ) {
                                            ...
                                            console.error(
                                                "❌ Errore: il nodo requireNode non ha argomenti!"
                                            );
                                        }
                                    }
                                } else if (
                                    ...
                                    if (
                                        ...
                                        pushBlock.data = JSON.stringify({
                                            parentId: pushBlock.parent,
                                        });
                                        blocklyJson.blocks.blocks.push(
                                            pushBlock
                                        );
                                        ...
                                        const structBlockId =
                                            generateUniqueId();
                                        const name_new_struct: string | null =
                                            arguments_[0].expression.name ??
                                            null;

                                        const structValues: Record<
                                            string,
                                            string | boolean
                                        > = {};

                                        if (
                                            arguments_[0].arguments?.[0]
                                                ?.type === "NamedArgument"
                                        ) {
                                            arguments_[0].arguments.forEach(
                                                (namedArg: any) => {
                                                    const key =
                                                        namedArg.name.name;
                                        ...
                                        const structBlock = {
                                            ...
                                            parent: pushBlock.id,
                                            data: JSON.stringify({
                                                parentId: pushBlock.id,
                                                values: structValues,
                                            }),
                                        };

                                        console.log(
                                            "→ StructValues da salvare:",
                                            JSON.stringify(
                                                structValues,
                                                null,
                                                2
                                    ...
                                    const InternalFuncBlockId: string =
                                        generateUniqueId();
                                    let output: string = "";
                                    ...
                                    const args: string =
                                        statement.expression.arguments
                                            ?.map(
                                                (arg: any) =>
                                                    arg.name ?? arg.value ?? ""
                                            ) // fallback su value se non ha name
                                            .join(", ") ?? "";
                                    ...
                                    InternalFuncBlock.data = JSON.stringify({
                                        parentId: InternalFuncBlock.parent,
                                    });
                                    blocklyJson.blocks.blocks.push(
                                        InternalFuncBlock
                                    );
                                ...
                                if (assignment.left.type === "MemberAccess") {
                                    ...
                                    const block = {
                                        ...
                                        fields: {
                                            VAR: assignment.left.expression
                                                .name,
                                            ATTRIBUTE:
                                                assignment.left.memberName,
                                    ...
                                    if (!blockType) {
                                        throw new Error(
                                            `❌ Tipo di variabile non gestito: ${varType}`
                                        );
                                    }
                                    ...
                                    setblock.data = JSON.stringify({
                                        parentId: setblock.parent,
                                    });
                                    ...
                                    if (assignment.operator === "+=") {
                                        ...
                                        if (
                                            ...
                                            const inputValue =
                                                assignment.right.right.value ||
                                                assignment.right.right.name;
                                            const inputName =
                                                assignment.right.left.name;
                                            ...
                                            assignment.right.baseExpression &&
                                            assignment.right.indexExpression
                                        ) {
                                    ...
                                    assignment.left &&
                                    assignment.left.type === "IndexAccess"
                                ) {
                                    let varName = "";
                                    let param1 = "";
                                    let param2 = "";
                                    ...
                                    const baseExpression =
                                        assignment.left.baseExpression;
                                    const indexExpression =
                                        assignment.left.indexExpression;
                                    ...
                                    switch (assignment.right?.type) {
                                        ...
                                        case "StringLiteral":
                                            param2 =
                                                assignment.right.value.toString();
                                            break;
                                    }

                                    if (
                                        indexExpression?.type === "Identifier"
                                    ) {
                                ...
                                // CASE ARRAY (Delete)
                                if (
                                    statement.expression.operator === "delete"
                                ) {
                                    const deleteBlockId: string =
                                        generateUniqueId();
                                    const deleteBlock: BlocklyBlock = {
                                        type: "array_delete",
                                        fields: {
                                            VAR: assignment.right.baseExpression
                                                .name,
                                            PARAMS1:
                                                assignment.right.indexExpression
                                                    .value,
                                    ...
                                    deleteBlock.data = JSON.stringify({
                                        parentId: deleteBlock.parent,
                                    });
                                    ...
                                    switch (varType) {
                                        ...
                                        default:
                                            throw new Error(
                                                `❌ Tipo variabile non gestito: ${varType}`
                                            );
                                    }
                                    ...
                                    if (
                                        ...
                                        throw new Error(
                                            `❌ Operatore unario non gestito: ${statement.expression.operator}`
                                        );
                                    }
                                ...
                                EmitEventBlock.data = JSON.stringify({
                                    parentId: EmitEventBlock.parent,
                                });
                                blocklyJson.blocks.blocks.push(EmitEventBlock);
                            } else if (statement.type === "ReturnStatement") {
                                const returnBlockId: string =
                                    generateUniqueId();
                                ...
                                if (!statement.expression) {
                                    ...
                                    returnBlock.data = JSON.stringify({
                                        parentId: returnBlock.parent,
                                    });
                                    ...
                                    statement.expression.type === "IndexAccess"
                                ) {
                                    const base: string =
                                        statement.expression.baseExpression
                                            .name;
                                    const index: string =
                                    ...
                                    const memberName: string =
                                        assignment.expression.memberName;
                                ...
                                if (
                                    ...
                                    statement.expression.type ===
                                        "FunctionCall" &&
                                    statement.expression.expression.type ===
                                        "NewExpr"
                                ) {
                                    ...
                                    const baseType: string =
                                        statement.variable.typeName.baseType
                                            .name;
                                    const varName: string =
                                        statement.variable.name.name;
                                    const newType: string =
                                        statement.expression.expression.typeName
                                            .baseType.name;
                                    ...
                                    const type: string =
                                        statement.variable.typeName.name;
                                    const name: string =
                                        statement.variable.name.name;
                                    const fnName: string =
                                        statement.expression.expression.name;
                                    ...
                                    locVariableblock.data = JSON.stringify({
                                        parentId: locVariableblock.parent,
                                    });
                                    blocklyJson.blocks.blocks.push(
                                        locVariableblock
                                    );
                                    ...
                                    const blackBlock: BlocklyBlock =
                                        generateBlackBlock(
                                            statement,
                                            functionBlock.id
                                        );
                                    blackBlock.data = JSON.stringify({
                                        parentId: blackBlock.parent,
                                    });
                                    blocklyJson.blocks.blocks.push(blackBlock);
                                }
                            } else if (statement.type === "IfStatement") {
                                let elseBlock: BlocklyBlock | undefined;
                                let if_elseIf_else_Block:
                                    | BlocklyBlock
                                    | undefined;
                                ...
                                if (statement.elseStatement) {
                                    ...
                                    if (count > 0) {
                                        const if_elseif_else_BlockId: string =
                                            generateUniqueId();
                                        ...
                                        if_elseIf_else_Block.data =
                                            JSON.stringify({
                                                parentId:
                                                    if_elseIf_else_Block.parent,
                                            });
                                        blocklyJson.blocks.blocks.push(
                                            if_elseIf_else_Block
                                        );
                                        console.log(
                                            "if_elseIf_else block added to json"
                                        );
                                        ...
                                        console.log(
                                            "IF block with the if_elseif_else ID"
                                        );
                                        ...
                                        elseBlock.data = JSON.stringify({
                                            parentId: elseBlock.parent,
                                        });
                                        blocklyJson.blocks.blocks.push(
                                            elseBlock
                                        );
                                        console.log(
                                            "Else block added to the json"
                                        );

                                        const sortedElseStatements =
                                            elseStatementnodes
                                                .slice()
                                                .sort(
                                                    (a, b) =>
                                        ...
                                        sortedElseStatements.forEach(
                                            (statement, i) => {
                                                const elseifBlockId: string =
                                                    generateUniqueId();
                                                const elseifBlock: BlocklyBlock =
                                                    {
                                                        type: "else_if",
                                                        id: elseifBlockId,
                                                        parent: if_elseIf_else_Block!
                                                            .id,
                                                        fields: {},
                                                    };
                                                    ...
                                                    if (
                                                        ...
                                                        if (type) {
                                                            ...
                                                            if (blockType) {
                                                                ...
                                                                console.log(
                                                                    "inputBlockRight for UnaryOperation created!"
                                                                );
                                                            }
                                                        }
                                                    } else if (
                                                            ...
                                                            if (
                                                                ...
                                                                if (
                                                                    ...
                                                                    if (
                                                                        ...
                                                                        console.log(
                                                                            "inputBlockRight for rightOperand equal to a variable created!"
                                                                        );
                                                                    }
                                                                }
                                                            } else {
                                                        ...
                                                        if (type !== null) {
                                                            ...
                                                            if (
                                                                ...
                                                                console.log(
                                                                    "inputBlockLeft for leftOperand equal to a variable created!"
                                                                );
                                                            }
                                                        } else {
                                                            let leftOperand: string =
                                                            ...
                                                            if (
                                                                ...
                                                                console.warn(
                                                                    "⚠️ leftOperand non riconosciuto:",
                                                                    left
                                                                );
                                                                leftOperand =
                                                                    "undefined";
                                                            ...
                                                            console.log(
                                                                "✅ inputBlockLeft for Binary Operation created!"
                                                            );
                                                        }
                                                    }
                                                }
                                                            ...
                                                            if (
                                                                ...
                                                                if (
                                                                    ...
                                                                    if (
                                                                        ...
                                                                        returnvalueBlock.data =
                                                                            JSON.stringify(
                                                                                {
                                                                                    parentId:
                                                                                        returnvalueBlock.parent,
                                                                                }
                                                                            );
                                                                        blocklyJson.blocks.blocks.push(
                                                                            returnvalueBlock
                                                                ...
                                                                if (
                                                                    ...
                                                                    console.log(
                                                                        "Terza condizione letta"
                                                                    );

                                                                    const assignment =
                                                                        statement.expression; //as IdentifierNode;
                                                                    const varName: string =
                                                                        assignment.name;
                                                                    ...
                                                                    const assignment =
                                                                        statement.expression; // as FunctionCallNode;
                                        ...
                                        (
                                            savedElseBlockStatements1 as any[]
                                        ).forEach((statement) => {
                                            (
                                                statement.elseStatement
                                                    .statements as any[]
                                                ...
                                                if (
                                                    ...
                                                    console.log(
                                                        "Prima condizione letta nell'else"
                                                    );
                                                    ...
                                                    console.log(
                                                        "Seconda condizione letta nell'else"
                                                    );
                                                    const assignment =
                                                        statement.expression;
                                                    const varName: string =
                                        ...
                                        count < 1 &&
                                        savedElseBlockStatements.length > 0
                                    ) {
                                        // Count < 1 && siamo nell'elseStatement
                                        // PARENT: if_else_Block BLOCK

                                        const if_else_BlockId: string =
                                            generateUniqueId();
                                        ...
                                        if_else_Block.data = JSON.stringify({
                                            parentId: if_else_Block.parent,
                                        });
                                        blocklyJson.blocks.blocks.push(
                                            if_else_Block
                                        );
                                        console.log(
                                            "if_else block added to json"
                                        );
                                        ...
                                        console.log(
                                            "ELSE block with the if_else ID"
                                        );
                                        ...
                                        savedElseBlockStatements.forEach(
                                            (statement) => {
                                    ...
                                    ifBlock.data = JSON.stringify({
                                        parentId: ifBlock.parent,
                                    });
                                ...
                                if (
                                    ...
                                    const conditionType: string =
                                        operatorMapping[solidityOperator] ||
                                        "EQUAL";
                                    ...
                                    if (solidityOperator === "!") {
                                        ...
                                        const typeFound = findVariableType(
                                            ast,
                                            right.name
                                        );
                                        if (typeFound) {
                                        ...
                                        if (right) {
                                            const inputBlockRightId: string =
                                                generateUniqueId();
                                ...
                                if (
                                    ...
                                    statement.ifStatement.statements
                                ) {
                                    const statements =
                                        statement.ifStatement.statements;
                                    const sortedIfStatements = statements
                                        .slice()
                                        .sort(
                                            (a: any, b: any) =>
                                                b.range[0] - a.range[0]
            ...
            case "VariableDeclaration": {
                ...
                if (
                    ...
                    console.log(
                        "📚 attributi definiti nello structRegistry:",
                        structDefinition
                    );

                    if (
                        Array.isArray(args) &&
                        Array.isArray(structDefinition)
                    ) {
                    ...
                    const variableBlock: any = {
                        ...
                        fields: {
                            ...
                            TYPE1: returnTypeMapping[
                                node.typeName.keyType.name
                            ],
                            TYPE2: returnTypeMapping[
                                node.typeName.valueType.name
                            ],
                            NAME: node.name.name,
                        },
                        id: mappingBlockId,
                    ...
                    variableBlock.data = JSON.stringify({
                        parentId: variableBlock.parent,
                    });
                    ...
                    if (returnTypeArray[node.typeName.name] === undefined) {
                        ...
                        if (node.expression?.type === "InlineArray") {
                            ...
                            const blocklyType =
                                solidityTypeToBlocklyType[arraySolidityType];

                            if (!blocklyType) {
                                console.warn(
                                    `Tipo ${arraySolidityType} non supportato.`
                                );
                                return;
                            }
                        ...
                        console.log(
                            "Case ASSIGNMENT OF VALUES TO AN ARRAY VARIABLE"
                        );

                        const arrayType: string = node.typeName.name || "";
                        const arrayName: string = node.name?.name || "";
                        const newExprType: string =
                            node.expression.expression.typeName?.name || "";
                        const values: string =
                    ...
                    const varImmutable: "yes" | "not" = immutable
                        ? "yes"
                        : "not";
                    ...
                    const payable: "yes" | "doesn't matter" = isPayable
                        ? "yes"
                        : "doesn't matter";
                    ...
                    if (node.expression !== null) {
                        ...
                        const subDenomination: string | undefined =
                            node.expression.subDenomination;
                        const valueCode: string = subDenomination
                            ? `${baseValue} ${subDenomination}`
                            : baseValue;

                        if (
                            ["string", "address", "bytes", "bytes32"].includes(
                                varType
                            )
                        ) {
                    ...
                    defineBlock.data = JSON.stringify({
                        parentId: defineBlock.parent,
                    });
            ...
            case "StructDefinition": {
                let structBlockId = generateUniqueId();
            ...
            case "StructMember": {
                ...
                console.log(
                    `StructMember - ID: ${memberBlockId}, Parent: ${parentBlockId}`
                );
                ...
                if (
                    ...
                    console.log(
                        `🎯 Creazione blocco: ID=${memberBlock.id}, Tipo=${memberBlock.type}, Parent=${memberBlock.parent}`
                    );
                } else {
            ...
            case "ModifierDefinition": {
                ...
                if (
                    ...
                    if (requireNode.arguments?.length > 0) {
                        ...
                        let conditionExpression = requireArg?.expressions
                            ?.length
                            ? requireArg.expressions[0]
                            : requireArg;
                        ...
                        if (
                            ...
                            conditionType =
                                operatorMapping[solidityOperator] || "EQUAL";
                            ...
                            if (solidityOperator === "!") {
                                ...
                                requireConditionBlock.data = JSON.stringify({
                                    parentId: requireConditionBlock.parent,
                                });
                                blocklyJson.blocks.blocks.push(
                                    requireConditionBlock
                                );
                                ...
                                if (right) {
                                    ...
                                    if (
                                        findVariable(ast, right.name) === true
                                    ) {
                                        const type = findVariableType(
                                            ast,
                                            right.name
                                        );
                                        console.log("name:", type);
                                        ...
                                        if (
                                            ...
                                            const base =
                                                right.baseExpression.name;
                                            const index =
                                                right.indexExpression.name;
                                    ...
                                    if (inputBlockRight) {
                                        inputBlockRight.data = JSON.stringify({
                                            parentId: inputBlockRight.parent,
                                        });
                                        blocklyJson.blocks.blocks.push(
                                            inputBlockRight
                                        );
                                ...
                                if (right) {
                                    ...
                                    if (findVariable(ast, right.name)) {
                                        ...
                                        if (type && type in typeMap) {
                                            ...
                                            if (blockType) {
                                                ...
                                                console.log(
                                                    "inputBlockRight for variable created!"
                                                );
                                            } else {
                                                throw new Error(
                                                    `❌ Tipo non supportato: ${type}`
                                    ...
                                    if (
                                        ...
                                        left.expression.type ===
                                            "FunctionCall" &&
                                        left.expression.expression.type ===
                                            "ElementaryTypeName" &&
                                        left.expression.expression.name ===
                                            "address" &&
                                        left.expression.arguments?.[0]?.type ===
                                            "Identifier" &&
                                        left.expression.arguments[0].name ===
                                            "this" &&
                                        left.memberName === "balance"
                                    ) {
                                        const leftOperand =
                                        ...
                                        if (type && type in typeToBlockMap) {
                                            ...
                                            throw new Error(
                                                `❌ Tipo non gestito o nullo per la variabile: ${type}`
                                            );
                                        }
                                    }

                                    // Salvataggio e aggiunta al JSON Blockly
                                    inputBlockLeft.data = JSON.stringify({
                                        parentId: inputBlockLeft.parent,
                                    });
                                    blocklyJson.blocks.blocks.push(
                                        inputBlockLeft
                                    );
                                    console.log(
                                        "✅ inputBlockLeft creato per BinaryOperation"
                                    );
                                }
                                break;
                            }
...
export function positionBlocks(blocks: Blockly.Block[]): void {
    ...
    function placeBlock(block: Blockly.Block, x: number, y: number) {
        block.moveBy(x, y);
        console.log(
            `Posizionato blocco ${block.type} (ID: ${block.id}) a X:${x}, Y:${y}`
        );
    }
    ...
    blocks.forEach((block) => {
        if (!block) return;
        console.log(
            `Blocchi in posizionamento: ID=${block.id}, Tipo=${block.type}, Parent=${block.data}`
        );

        if (["structure"].includes(block.type)) {
            placeBlock(block, x, y);
            console.log(
                `Parent ${block.type} (ID: ${block.id}) posizionato a X:${x}, Y:${y}`
            );
        }
    });
    ...
    blocks.forEach((block) => {
        ...
        try {
            ...
            console.warn(
                `⚠️ Errore nel parsing dei dati del blocco ${block.id}:`,
                e
            );
        }

        console.log(
            `Blocco ID=${block.id}, Tipo=${block.type}, Parent=${parentId}, Section=${section}`
        );

        if (block.type === "contract" || block.type === "import") {
            const parent = blocks.find(
                (b) => b.type === "structure" && b.id === parentId
            );
            if (!parent) return;
            ...
            block.type === "mapping" ||
            block.type === "assign_values_to_variable_array" ||
            block.type === "define_variable" ||
            block.type === "array" ||
            block.type === "define_variable_with_assignment" ||
            block.type === "define_struct_variable_with_assignment" ||
            block.type === "define_variable_with_assignment1" ||
            block.type === "method" ||
            block.type === "contract_constructor" ||
            block.type === "modifier1" ||
            block.type === "event" ||
            block.type === "contract_structures"
        ) {
            const parent = blocks.find(
                (b) => b.type === "contract" && parentId === b.id
            );
            if (parent) {
                ...
                if (inputVariables && inputVariables.connection) {
                    ...
                    if (
                        ...
                        if (block.previousConnection) {
                            inputVariables.connection.connect(
                ...
                if (inputStructs && inputStructs.connection) {
                    ...
                    if (
                        ...
                        if (block.previousConnection) {
                            inputStructs.connection.connect(
                                block.previousConnection
                            );
                        }
                    }
                ...
                if (inputMappings && inputMappings.connection) {
                    ...
                    if (block.type === "mapping") {
                        if (block.previousConnection) {
                            inputMappings.connection.connect(
                                block.previousConnection
                            );
                        }
                    }
                ...
                if (inputEvents && inputEvents.connection) {
                    ...
                    if (block.type === "event") {
                        if (block.previousConnection) {
                            inputEvents.connection.connect(
                                block.previousConnection
                            );
                        }
                    }
                    ...
                    arrayBlocks.forEach((arrayBlock, i) => {
                        if (i === 0) {
                            if (arrayBlock.previousConnection) {
                                inputArrays.connection!.connect(
                                    arrayBlock.previousConnection
                                );
                            }
                        } else if (
                            ...
                            prevBlock.nextConnection.connect(
                                arrayBlock.previousConnection
                            );
                        }
                ...
                if (inputConstructor && inputConstructor.connection) {
                    ...
                    if (block.type === "contract_constructor") {
                        if (block.previousConnection) {
                            inputConstructor.connection.connect(
                                block.previousConnection
                            );
                        }
                    }
                ...
                if (inputModifiers && inputModifiers.connection) {
                    ...
                    if (block.type === "modifier1") {
                        if (block.previousConnection) {
                            inputModifiers.connection.connect(
                                block.previousConnection
                            );
                        }
                    }
                ...
                if (inputmethods?.connection) {
                    ...
                    if (
                        ...
                        if (block.previousConnection) {
                            inputmethods.connection.connect(
                                block.previousConnection
                            );
                        }
                    }
            ...
            const parent = blocks.find(
                (b: Blockly.Block) =>
                    [
                        "define_variable",
                        "require_condition",
                        "define_variable_with_assignment",
                        "define_variable_with_assignment1",
                    ].includes(b.type as string) && parentId === b.id
            );
            ...
            if (parent) {
                if (parent.type === "define_variable") {
                    ...
                    if (inputValue?.connection && block.outputConnection) {
                        console.log(
                            `Collegamento getter ${block.id} a define_variable ${parent.id}`
                        );
                    ...
                    if (input?.connection && block.outputConnection) {
                        console.log(
                            `Collegamento getter ${block.id} a define_variable_WITH_ASS ${parent.id}`
                        );
                    ...
                    if (input1?.connection && block.outputConnection) {
                        console.log(
                            `Collegamento getter ${block.id} a define_variable_WITH_ASS_1 ${parent.id}`
                        );
                    ...
                    if (inputValueLeft?.connection) {
                        const connectedBlockL =
                            inputValueLeft.connection.targetBlock();
                        if (!connectedBlockL && block.outputConnection) {
                            console.log(
                                `Collegamento input ${block.id} a LEFT di require_condition ${parent.id}`
                            );
                            inputValueLeft.connection.connect(
                    ...
                    if (inputValueRight?.connection) {
                        const connectedBlockR =
                            inputValueRight.connection.targetBlock();
                        if (!connectedBlockR && block.outputConnection) {
                            console.log(
                                `Collegamento input ${block.id} a RIGHT di require_condition ${parent.id}`
                            );
                            inputValueRight.connection.connect(
                                block.outputConnection
                            );
                        }
                    }
            ...
            const parent = blocks.find(
                (b): b is Blockly.Block =>
                    ["method", "if"].includes(b.type) && parentId === b.id
            );
            ...
            if (parent) {
                if (parent.type === "method") {
                    ...
                    if (inputReturn?.connection && block.previousConnection) {
                        inputReturn.connection.connect(
                            block.previousConnection
                        );
                    }
                    ...
                    ["controls_if", "controls_ifelse"].includes(parent.type)
                ) {
                    let n = 0;
                    ...
                    while ((doInput = parent.getInput("DO" + n))) {
                        if (
                            doInput.connection &&
                            !doInput.connection.isConnected() &&
                            block.previousConnection
                        ) {
            ...
            block.type === "internalFunc" ||
            block.type === "localVariable"
        ) {
            const parent = blocks.find(
                (b): b is Blockly.Block =>
                    b.type === "method" && parentId === b.id
            );
            if (parent) {
            ...
            const parent = blocks.find(
                (b): b is Blockly.Block =>
                    ["method", "if", "else", "else_if"].includes(b.type) &&
                    parentId === b.id
            );
            if (parent) {
                if (["else_if", "if", "else"].includes(parent.type)) {
                    ...
                    if (inputMethod?.connection && block.previousConnection) {
                        inputMethod.connection.connect(
                            block.previousConnection
                        );
                    }
            ...
            const parent = blocks.find(
                (b): b is Blockly.Block =>
                    [
                        "if_else_container",
                        "if_elseif_else_container",
                        "method",
                    ].includes(b.type) && parentId === b.id
            );
            if (parent) {
                if (
                    ["if_else_container", "if_elseif_else_container"].includes(
                        parent.type
                    )
                ) {
            ...
            if (parent) {
                ...
                if (inputReturn?.connection) {
                    const arrayBlocks = blocks.filter((b) =>
                        [
                            "define_arrayVariable",
                            "array_pop",
                            "new_struct_value",
            ...
            const parent = blocks.find(
                (b): b is Blockly.Block =>
                    [
                        "struct_push",
                        "define_struct_variable_with_assignment",
                    ].includes(b.type) && parentId === b.id
            );
            if (parent) {
                if (parent.type === "struct_push") {
                    ...
                    parent.type === "define_struct_variable_with_assignment"
                ) {
            ...
            block.type === "getter_mappings" ||
            block.type === "array_values"
        ) {
            const parent = blocks.find(
                (b): b is Blockly.Block =>
                    b.type === "method" && parentId === b.id
            ...
            const parent = blocks.find(
                (b): b is Blockly.Block =>
                    b.type === "if_elseif_else_container" && parentId === b.id
            );
            if (parent) {
            ...
            block.type === "if_else_container" ||
            block.type === "if_elseif_else_container"
        ) {
            const parent = blocks.find(
                (b) => b.type === "method" && parentId === b.id
            );
            ...
            block.type === "func_inputs" ||
            block.type === "func_inputs_black"
        ) {
            const parent = blocks.find(
                (b) =>
                    [
            ...
            if (parent) {
                ...
                if (inputReturnValues?.connection && block.previousConnection) {
                    inputReturnValues.connection.connect(
                        block.previousConnection
                    );
                }
            }
        } else if (
            block.type === "struct_variables" ||
            block.type === "structVariables_black"
        ) {
            //console.log(`🔍 Prima del posizionamento: ID=${block.id}, Tipo=${block.type}, Parent=${block.parent}`);
            const parent = blocks.find(
                (b) => b.type === "contract_structures" && parentId === b.id
            );
            if (parent) {
                console.log(
                    `Trovato parent: ${parent.id}, Tipo: ${parent.type}`
                );
            } else {
                console.log(
                    `❌ Parent non trovato per struct_variables ${block.id}`
                );
            }
            ...
            if (parent) {
                if (
                    ["method", "contract_constructor", "modifier1"].includes(
                        parent.type
                    )
                ) {
            ...
            const parent = blocks.find(
                (b) => b.type === "require_condition" && parentId === b.id
            );
            ...
            block.type === "input_somma" ||
            block.type === "input_diff"
        ) {
            const parent = blocks.find(
                (b) =>
                    [
            ...
            if (parent) {
                if (
                    ...
                    if (conditionInput?.connection && block.outputConnection) {
                        conditionInput.connection.connect(
                            block.outputConnection
                        );
                    }
                }
